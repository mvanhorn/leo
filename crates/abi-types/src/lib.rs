...
impl Function {
    /// Returns `true` if the function is async — its last output is an Aleo `future`. Such a
    /// function compiles with a `finalize` scope.
    pub fn is_final(&self) -> bool {
        self.outputs.iter().any(|o| matches!(o, FunctionOutput::Final))
    }
}
