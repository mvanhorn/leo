...
pub(crate) fn not_compatible(program: impl Display, standard: impl Display, unsatisfied: usize) -> Backtraced {
    Backtraced::error(
        CODE_PREFIX,
        CODE_MASK + 69,
        format!("program `{program}` is not compatible with standard `{standard}`: {unsatisfied} interface item(s) unsatisfied"),
    )
    .with_help("Review the unsatisfied items listed above. The program must declare every function, view, mapping, storage variable, record, and struct the standard requires, with matching signatures.")
}
