...
fn build_interface(
    ...
    let mut interface = abi::Interface {
        name,
        program,
        path,
        parents,
    ...
    // A reference to a type defined in this program/library is local, so it carries no qualifier.
    crate::localize_interface_refs(&mut interface);
