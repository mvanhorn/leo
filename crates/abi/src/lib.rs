...
pub fn generate(ast: &ast::Program) -> abi::Program {
    ...
    // A reference to a type defined in this program is local, so it carries no program qualifier.
    localize_program_refs(&mut program);
...
/// Rewrites every reference to a type defined in this program so it carries no `program`
/// qualifier (local types have `program: None`; only external references keep `Some`).
pub(crate) fn localize_program_refs(program: &mut abi::Program) {
    let id = program.program.clone();
    localize_items(
        &id,
        &mut program.structs,
        &mut program.records,
        &mut program.mappings,
        &mut program.storage_variables,
        &mut program.functions,
        &mut program.views,
    );
}

/// Like [`localize_program_refs`], but for an interface ABI (refs local to the owning program or
/// library lose their qualifier).
pub(crate) fn localize_interface_refs(interface: &mut abi::Interface) {
    let id = interface.program.clone();
    localize_items(
        &id,
        &mut interface.structs,
        &mut interface.records,
        &mut interface.mappings,
        &mut interface.storage_variables,
        &mut interface.functions,
        &mut interface.views,
    );
}
...
fn localize_items(
    id: &str,
    structs: &mut [abi::Struct],
    records: &mut [abi::Record],
    mappings: &mut [abi::Mapping],
    storage: &mut [abi::StorageVariable],
    functions: &mut [abi::Function],
    views: &mut [abi::Function],
) {
    for s in structs {
        for field in &mut s.fields {
            localize_plaintext(&mut field.ty, id);
        }
    }
    for r in records {
    ...
    for m in mappings {
        localize_plaintext(&mut m.key, id);
        localize_plaintext(&mut m.value, id);
    }
    for sv in storage {
        localize_storage(&mut sv.ty, id);
    }
    for f in functions.iter_mut().chain(views.iter_mut()) {
        for input in &mut f.inputs {
            match input {
                abi::FunctionInput::Plaintext { ty, .. } => localize_plaintext(ty, id),
                abi::FunctionInput::Record(r) => localize_ref(&mut r.program, id),
                abi::FunctionInput::DynamicRecord => {}
            }
        }
        for output in &mut f.outputs {
            match output {
                abi::FunctionOutput::Plaintext { ty, .. } => localize_plaintext(ty, id),
                abi::FunctionOutput::Record(r) => localize_ref(&mut r.program, id),
                abi::FunctionOutput::Final | abi::FunctionOutput::DynamicRecord => {}
            }
        }
    }
...
fn localize_plaintext(ty: &mut abi::Plaintext, id: &str) {
    match ty {
        abi::Plaintext::Struct(r) => localize_ref(&mut r.program, id),
        abi::Plaintext::Array(a) => localize_plaintext(&mut a.element, id),
        abi::Plaintext::Optional(o) => localize_plaintext(&mut o.0, id),
        abi::Plaintext::Primitive(_) => {}
    }
}

fn localize_storage(ty: &mut abi::StorageType, id: &str) {
    match ty {
        abi::StorageType::Plaintext(p) => localize_plaintext(p, id),
        abi::StorageType::Vector(inner) => localize_storage(inner, id),
    }
}

fn localize_ref(program: &mut Option<String>, id: &str) {
    if program.as_deref() == Some(id) {
        *program = None;
    }
}
